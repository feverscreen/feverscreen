...
export class Face {
  ...
  setHotspot(source: Float32Array, frameWidth: number) {
    let r;
    if (this.tracked()) {
  ...
  detectForehead(
    ...
    const [faceX, endY, xFeatures] = this.xScan(source, roi, roi, frameWidth);
    const [faceY, yFeatures] = this.yScan(
      source,
      faceX,
      roi,
      endY,
  ...
  yScan(
    ...
    for (let x = ~~faceX.x0 + 1; x < ~~faceX.x1 - 1; x++) {
      ...
      for (let y = ~~roi.y0 + 1; y < endY; y++) {
        ...
        const index = y * frameWidth + x;
        const [intensity, direction] = sobelEdge(source, index, frameWidth);

        const edge = detectYEdge(faceY, y, intensity, direction);
  ...
  xScan(
    ...
    roi: ROIFeature,
    frameWidth: number
  ): [ROIFeature, number | null, ROIFeature[]] {
    let longestLine;
    let maxY = null;
    const xFeatures = [];
    ...
    for (let y = ~~faceY.y0 + 1; y < ~~faceY.y1 - 1; y++) {
      ...
      for (let x = ~~roi.x0 + 1; x < ~~roi.x1 - 1; x++) {
        ...
        const index = y * frameWidth + x;
        const [intensity, direction] = sobelEdge(source, index, frameWidth);
        const edgeDetected = detectXEdge(faceX, x, intensity, direction);
    ...
    return [longestLine || roi, maxY, xFeatures];
  }
}
