...
function moduleTemperatureAnomaly(timeSinceFFC) {
...
function sensorAnomaly(timeSinceFFC) {
    ...
    return {
        arrayBuffer(blob) {
            return new Promise((resolve, reject) => {
                ...
                fileReader.addEventListener('load', (_event) => {
                    resolve(fileReader.result);
                });
                fileReader.addEventListener('error', (_event) => {
                    reject();
                });
...
window.onload = async function () {
    ...
    let GCalibrate_body_location = TemperatureSource.FOREHEAD;
    let fovBox = {
        top: 0,
        right: 0,
        bottom: 0,
    ...
    const temperatureInputCelsius = document.getElementById("temperature_input_a");
    ...
    setTitle("Loading");
    function onResizeViewport(e = undefined) {
    ...
    let nativeOverlayHeight;
    {
        ...
        const startDrag = (eventName) => (e) => {
            currentTarget = e.currentTarget;
            window.addEventListener(eventName, dragHandle);
        };
        const endDrag = (eventName) => () => {
    ...
    function showTemperature(temperature_celsius, uncertainty_celsius) {
        ...
        temperatureDiv.classList.remove("error-state", "error2-state", "normal-state", "fever-state");
    ...
    function median_smooth_pass(source, delta, swizzle) {
        ...
        for (let y = y0; y !== y1; y += dy) {
            for (let x = x0; x !== x1; x += dx) {
                ...
                const value = median_three(source[index - delta], current, source[index + delta]);
    ...
    function accumulate_stable_temperature(source) {
    ...
    function processSnapshotRaw(source, frameInfo, timeSinceFFC) {
        {
    ...
    function drawOverlay() {
        ...
        overlayCtx.arc((hotSpotX * nativeOverlayWidth) / canvasWidth, (hotSpotY * nativeOverlayHeight) / frameHeight, 30 * window.devicePixelRatio, 0, 2 * Math.PI, false);
    ...
    const retrySocket = (retryTime, deviceIp) => {
        if (retryTime > 0) {
    ...
    async function updateFrame(data, frameInfo) {
        clearTimeout(animatedSnow);
        ...
        const { Telemetry: telemetry, Calibration: calibration, Camera: camera, BinaryVersion, AppVersion } = frameInfo;
...
//# sourceMappingURL=feverscreen.js.map